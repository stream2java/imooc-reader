    ...
    <div class="container mt-2 p-2 m-0">
        <form id="frmLogin">
            <div class="passport bg-white">
                <h4 class="float-left">管理員登入</h4>
<#--                <h6 class="float-right pt-2"><a href="/register.html">會員註冊</a></h6>-->
                ...
                <a id="btnSubmit" class="btn btn-success  btn-block mt-4 text-white pt-3 pb-3">登   入</a>
            </div>
        </form>
...
<script>
    ...
    $("#btnSubmit").click(function () {
        ...
        if (!regex.test(username)) {
            showTips(true, "alert-danger", "用戶名請輸入正確格式（1-10位）");
            return;
        } else {
            showTips(false);
        }
        ...
        if (!regex.test(password)) {
            showTips(true, "alert-danger", "密碼請輸入正確格式（1-10位）");
            return;
        } else {
            showTips(false);
        }
